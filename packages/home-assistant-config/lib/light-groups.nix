[
  {
    key = "office";
    name = "Office";
    button_devices = ["8752b03db77ab3ffb1c2804af0ef96e0"];
    light_targets = ["light.office_light"];
    switch_targets = [];
  }
  {
    key = "kitchen";
    name = "Kitchen";
    button_devices = ["f1674dd725bd9c3d13a2e36edb9e8840"];
    light_targets = ["light.kitchen_light"];
    switch_targets = [];
  }
  {
    key = "living_room";
    name = "Living Room";
    button_devices = ["f52de2f5b2f5a945fcc415f6c941b9e1"];
    light_targets = [
      "light.living_room_west_light"
      "light.living_room_east_light"
      "light.living_room_west_lamp"
    ];
    switch_targets = [
      "switch.living_room_east_lamp"
    ];
  }
  {
    key = "hallway";
    name = "Hallway";
    button_devices = [
      "b5987af5c927684ff7c285aa47d77c3a"
      "466f06365a6e3663f7f428ad9e4ddc02"
      "4457a4e998c46cafd3a0fb5f269cef5c"
    ];
    light_targets = [
      "light.hallway_0_light"
      "light.hallway_1_light"
      "light.hallway_2_light"
    ];
    switch_targets = [];
  }
  {
    key = "bedroom";
    name = "Bedroom";
    button_devices = [
      "e9afa6047adf3f9375ebbe8958f0e89e"
    ];
    light_targets = [
      "light.bedroom_light"
    ];
    switch_targets = [
      "switch.bedroom_lamp"
    ];
  }
  {
    key = "guest_room";
    name = "Guest Room";
    button_devices = [
      "622b4c44071820e19758c9929a136aff"
    ];
    light_targets = [
      "light.guest_room_light"
    ];
    switch_targets = [];
  }
  {
    key = "bathroom";
    name = "Bathroom";
    button_devices = [
      "803435a0c5826acd45bdb95c57fe9d0a"
    ];
    light_targets = [
      "light.bathroom_light"
      "light.bathroom_mirror_light"
    ];
    switch_targets = [];
  }
  {
    key = "attic";
    name = "Attic";
    button_devices = [
      "98dbf93aad257dd626c15277b0e136d3"
    ];
    light_targets = [
      "light.attic_light"
    ];
    switch_targets = [];
  }
]