...
let
  ...
  cfg = blCfg.generic-extlinux-compatible-improved;
...
{
  options = {
    boot.loader.generic-extlinux-compatible-improved = {
      enable = mkOption {
        default = false;
        type = types.bool;
        description = ''
  ...
  in
    mkIf cfg.enable {
      ...
      system.boot.loader.id = "generic-extlinux-compatible-improved";

      boot.loader.generic-extlinux-compatible-improved.populateCmd = "${populateBuilder} ${builderArgs}";

      assertions = [
        {
          assertion = cfg.mirroredBoots != [ ];
          message = ''
            You must not remove all elements from option 'boot.loader.generic-extlinux-compatible-improved.mirroredBoots',
