  ...
  in {
    "sqm-ifb-redirect-${networkName}" = {
      ...
      script = ''
        ...
        ${pkgs.iproute2}/bin/tc qdisc add dev ${ifb} root cake bandwidth ${network.bw-ingress}bit ${optionalString (network.bw-ingress-auto or false) "autorate-ingress"} nat diffserv3
      '';
